...
function computerPlay() {
  const weapons = ["fire", "water", "wind"];
  const computerSelection = weapons[Math.floor(Math.random() * weapons.length)];
  const computerIcon = document.querySelector(".computer-icon");

  computerIcon.classList.remove("fa-skull", "fa-fire", "fa-water", "fa-wind");
  if (computerSelection === "fire") {
    computerIcon.classList.add("fa-fire");
    computerIcon.style.color = "#e25822";
  } else if (computerSelection === "water") {
    computerIcon.classList.add("fa-water");
    computerIcon.style.color = "aqua";
  } else if (computerSelection === "wind") {
    computerIcon.classList.add("fa-wind");
    computerIcon.style.color = "#3b3b3b";
  }
...
function countLives(playerSelection, computerSelection) {
  ...
  switch (true) {
    case playerSelection === computerSelection:
      ...
      computerPlayDiv.classList.remove(
        "grey-border",
        "green-border",
        "red-border"
      );
      ...
      break;
    case playerSelection === "fire" && computerSelection === "wind":
    case playerSelection === "water" && computerSelection === "fire":
    case playerSelection === "wind" && computerSelection === "water":
    ...
    default:
      ...
      gameOutput.style.border = "4px solid #3b3b3b";
      computerPlayDiv.classList.remove(
        "grey-border",
        "green-border",
        "purple-border"
